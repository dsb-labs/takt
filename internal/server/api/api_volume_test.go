	...
	tt := []struct {
	...
	}{
		{
			...
			SetupMocks: func(svc *MockVolumeService) {
				svc.EXPECT().Apply(mock.Anything, mock.MatchedBy(func(v manifest.Volume) bool { return v.Name == "example-data" }), 0).
...
func TestVolumeAPI_ListVolumes_Query(t *testing.T) {
	t.Parallel()
