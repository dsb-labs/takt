...
type (
	...
	Port struct {
		...
		WorkloadID string
		// The index of the workload instance the port is allocated to. Instances
		// are numbered from zero, so a workload running one instance allocates
		// only instance zero.
		Instance int
...
func (r *PortRepository) List(ctx context.Context, workloadID string) ([]Port, error) {
	const q = `
		SELECT workload_id, instance, name, container_port, host_port, protocol, is_dynamic
		FROM workload_port
		WHERE workload_id = ?
		ORDER BY instance ASC, container_port ASC, protocol ASC
	`
	...
	for rows.Next() {
		var port Port
		if err = rows.Scan(&port.WorkloadID, &port.Instance, &port.Name, &port.Container, &port.Host, &port.Protocol, &port.Dynamic); err != nil {
...
func (r *PortRepository) ListAll(ctx context.Context) (map[string][]Port, error) {
	const q = `
		SELECT workload_id, instance, name, container_port, host_port, protocol, is_dynamic
		FROM workload_port
		ORDER BY workload_id ASC, instance ASC, container_port ASC, protocol ASC
	`
...
func claim(ctx context.Context, tx *sql.Tx, workloadID string, ports []Port) error {
	const (
		...
		insert = `
			INSERT INTO workload_port (workload_id, instance, name, container_port, host_port, protocol, is_dynamic)
			VALUES (?, ?, ?, ?, ?, ?, ?)
		`
	)
	...
	for _, port := range ports {
		_, err := tx.ExecContext(ctx, insert, workloadID, port.Instance, port.Name, port.Container, port.Host, port.Protocol, port.Dynamic)
		switch {
