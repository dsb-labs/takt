...
func TestWrap(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.Name, func(t *testing.T) {
			...
			middleware.Wrap(handler, middleware.Config{Logger: logger}).ServeHTTP(resp, req)
...
func TestWrap_LetsAHandlerFlush(t *testing.T) {
	...
	middleware.Wrap(handler, middleware.Config{Logger: logger}).ServeHTTP(httptest.NewRecorder(), req)
...
func TestWrap_RecoveryIsLogged(t *testing.T) {
	...
	middleware.Wrap(http.HandlerFunc(func(http.ResponseWriter, *http.Request) {
		panic("something a handler did not expect")
	}), middleware.Config{Logger: logger}).ServeHTTP(resp, req)
