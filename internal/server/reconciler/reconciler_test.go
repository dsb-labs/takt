...
func TestReconciler_Run_ResolvesSecrets(t *testing.T) {
	...
	t.Run("hands the driver the resolved environment", func(t *testing.T) {
		...
		secrets.EXPECT().Resolve(mock.Anything, map[string]string{"DSN": "${secret:db-password}"}, mock.Anything, mock.Anything, mock.Anything).
	...
	t.Run("does not abandon ports when a secret cannot be resolved", func(t *testing.T) {
		...
		resolves := newCounter()
		secrets.EXPECT().Resolve(mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).
			RunAndReturn(func(context.Context, map[string]string, string, string, int) (map[string]string, error) {
				resolves.inc()
